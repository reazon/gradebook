...
//DN 2013-06-13: var event Blink(use for remove event Blink)
var eventBlink = Array();
...
function removeLinkUpdate(iRubricIconId){
    ...
    //DN 2013-06-13: remove blink icon refresh
    var row = getNumRow(iRubricIconId);
    if(eventBlink[row] != null)
      clearInterval(eventBlink[row]);
}
...
//DN 2013-06-13: function Blink for refresh Icon
function blink(iRubricIconId, interval){ 
    var element = document.getElementById(getFieldToUpdateElement(iRubricIconId,"getgrade"));
    //if have define element the refesh icon
    if(element != null) {
      //get number row
      var row = getNumRow(iRubricIconId);
      //add event Blink
      eventBlink[row] = window.setInterval(function(){
...
//DN 2013-06-13: set opacity
// value(by firefox): 
function setOpacity(element, value){
    //firefox, Chrome
    element.style.opacity = value;
    //IE 
    var valueIE = value * 100;
    element.style.filter = "alpha(opacity=" + valueIE +")";
}

//DN 2013-06-13:get row number(row set from 0)
function getNumRow(iRubricIconId){
    var pos = iRubricIconId.split(":");
    return pos[2]; //return number row
}

//DN 2013-06-13: clear blink for all(use for click the refesh icon in header)
function clearBlinkAll(){
  for(var i = 0; i < eventBlink.length; i++){
    if(eventBlink[i] != null)
      clearInterval(eventBlink[i]);
  }
} 
//DN 2013-06-13: show tool tip when click grade all
function renderToolTipGradeAll(iRubricIconId, event) {
    renderLinkUpdate(iRubricIconId, event, 5);
}

// render the refresh grade link 
function renderLinkUpdate(iRubricIconId, event, valueY){
 var doc = document;
 ...
 if(updateLink == null){
   ...
   var x = event.clientX + 215;
   var y = event.clientY + 65;
   //DN 2013-06-13: add valueY help we can see the refresh icon(show tooltip when click grade all)
   if(valueY != null)
        y = y - valueY;
   ...
   setTimeout(function(){
     ...
     //show tool tip
     $("body", parent.document).append('<div id="portalMask" style="filter:alpha(opacity=100);opacity:1;height:16px;border:1px solid;color:red;top:'+ 
       y +'px;left:'+ x +
         'px;">Please grade with iRubric, then click here to refresh grades.</div>');
     $("#portalMask").css("opacity", "1");
  ...
  //DN 2013-06-13: add event Blink to the refresh Icon 
  blink(iRubricIconId, 1000);  
 } 
}
